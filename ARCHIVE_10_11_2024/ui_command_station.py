...
def ui_loop():
    ...
    while running:
            ...
            scaled_button2 = assemble_button.inflate((scaled_button2.width * (button_scale_factor - 1)), (scaled_button2.height * (button_scale_factor - 1))) if is_button_pressed else assemble_button
            ...
            for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                            ...
                            if assemble_button.collidepoint(event.pos):
                                is_assemble_pressed = True  # Start button animation
                            if event.button == 1:
                                    if dragging_block:
                                            dragged_onto_something = False
                                            for zone_name, zone_rect in zones.items():
                                                    if zone_rect.colliderect(dragging_block.rect):
                                                            if zone_name == "middle":
                                                                    ...
                                                                    for j in range(num_slots): # check each slot
                                                                            ...
                                                                            if slot_rect.colliderect(dragging_block.rect):
                                                                                    ...
                                                                                    break
                                                                    if collision:
                                                                        if not dragging_block in queue_positions:
                                                            ...
                                                            break
                                            if not dragged_onto_something:
                                                if dragging_block.color in block_positions:
                                    ...
                                    is_assemble_pressed = False  # Reset button animation
