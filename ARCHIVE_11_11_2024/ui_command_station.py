            ...
            for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                            ...
                            if event.button == 1:
                                                    ...
                                                    if zone_rect.colliderect(dragging_block.rect):
                                                            ...
                                                            if zone_name == "middle":
                                                                for i in range(num_slots):
                                                                        ...
                                                                        line_x = circle_center[0] + int((circle_radius + line_length) * math.cos(angle))
                                                                        line_y = circle_center[1] + int((circle_radius + line_length) * math.sin(angle))
                                                                        print(i in block_positions.values(),i)
                                                                        if dragging_block.rect.collidepoint((line_x, line_y)) and (not color_names[dragging_block.color] in block_positions or ui_stage != 1) and not i+1 in block_positions.values():              # successful drop onto bay
                                    ...
                                    if go_button.collidepoint(event.pos) and len(block_positions)>0:
                                    ...
                                    if manual_button.collidepoint(event.pos):
                                            is_manual_pressed = True
                                            screen.blit(font.render("MANUAL... (press e to end)", True, BLACK), (470, 550))
                                            screen.blit(fontSmall.render("KEYS: 1-2 turntable voltage | 3-4 straight/flip | 5-9+0 bays 1-6 ", True, BLACK), (10, 150))
